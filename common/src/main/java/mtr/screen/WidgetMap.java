...
public class WidgetMap implements Renderable, NarratableEntry, GuiEventListener, IGui {
	...
	private boolean mouseDownMoved;
	private Tuple<Double, Double> pendingMapClick;
	...
	private final Consumer<Tuple<Double, Double>> onMapClicked; // 新增字段
	...
	public WidgetMap(TransportMode transportMode, OnDrawCorners onDrawCorners, Runnable onDrawCornersMouseRelease, Consumer<Long> onClickAddPlatformToRoute, Consumer<SavedRailBase> onClickEditSavedRail, BiFunction<Double, Double, Boolean> isRestrictedMouseArea, Consumer<Tuple<Double, Double>> onMapClicked) { // 构造函数新增参数
		...
		this.onMapClicked = onMapClicked; // 赋值新增字段
	...
	public boolean mouseReleased(MouseButtonEvent event) {
		...
		if (!mouseDownMoved && pendingMapClick != null) {
			onMapClicked.accept(pendingMapClick);
		}
	...
	public boolean mouseClicked(MouseButtonEvent event, boolean isDoubleClick) {
		if (isMouseOver(event.x(), event.y())) {
			...
			if (ClientData.hasPermission()) {
				mouseDownMoved = false;
				boolean isOnSavedRail = false;
				if (mapState == MapState.EDITING_AREA) {
					drawArea1 = coordsToWorldPos((int) (event.x() - x), (int) (event.y() - y));
					drawArea2 = null;
				} else if (mapState == MapState.EDITING_ROUTE) {
					isOnSavedRail = mouseOnSavedRail(mouseWorldPos, (savedRail, x1, z1, x2, z2) -> onClickAddPlatformToRoute.accept(savedRail.id), true);
				} else {
					isOnSavedRail = mouseOnSavedRail(mouseWorldPos, (savedRail, x1, z1, x2, z2) -> onClickEditSavedRail.accept(savedRail), showStations);
				}
				pendingMapClick = mapState == MapState.DEFAULT && !isOnSavedRail ? mouseWorldPos : null;
			}
			return true;
		} else {
			return false;
	...
	private boolean mouseOnSavedRail(Tuple<Double, Double> mouseWorldPos, MouseOnSavedRailCallback mouseOnSavedRailCallback, boolean isPlatform) {
