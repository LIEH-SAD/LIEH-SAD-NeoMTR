...
public class DashboardScreen extends MTRScreen implements IGui, IPacket {
	...
	private static final Identifier TELEPORT_BAR_TEXTURE = MTR.id("textures/gui/teleportbar.png");
	private boolean showTeleportBar = false;
	private static final float TELEPORT_BAR_FADE_TIME = 0.05F;
	private float teleportBarAlpha = 0.0F;
	private boolean teleportBarMouseDragged = false;
	private boolean teleportBarWasOpenOnPress = false;
	private boolean suppressTeleportBarOpen = false;
	private Tuple<Double, Double> teleportBarTargetPos;
	private String teleportBarTargetText = "";
	...
	private final Button buttonLineMap;
	private final Button buttonRailActions;
	private final Button buttonResourcePack;
	...
	public DashboardScreen(TransportMode transportMode, boolean useTimeAndWindSync) {
		...
		widgetMap = new WidgetMap(transportMode, this::onDrawCorners, this::onDrawCornersMouseRelease, this::onClickAddPlatformToRoute, this::onMapPointClicked, colorSelector::isMouseOver, this::onGeneralMapClicked);
		...
		buttonLineMap = UtilitiesClient.newButton(Text.translatable("gui.mtr.line_map"), button ->
				Util.getPlatform().openUri("https://localhost:8888")
		);
		...
		buttonResourcePack = UtilitiesClient.newButton(Text.translatable("menu.resource_pack"), button ->
		...
		buttonOptions = UtilitiesClient.newButton(Text.translatable("menu.options"), button ->
            Minecraft.getInstance().setScreen(ConfigScreen.createScreen(this))
        );
	...
	protected void init() {
		...
		IDrawing.setPositionAndWidth(buttonZoomIn, width - SQUARE_SIZE * 11, bottomRowY - SQUARE_SIZE, SQUARE_SIZE);
		IDrawing.setPositionAndWidth(buttonZoomOut, width - SQUARE_SIZE * 11, bottomRowY, SQUARE_SIZE);
		IDrawing.setPositionAndWidth(buttonLineMap, width - SQUARE_SIZE * 10, bottomRowY - SQUARE_SIZE, SQUARE_SIZE * 5);
		IDrawing.setPositionAndWidth(buttonRailActions, width - SQUARE_SIZE * 10, bottomRowY, SQUARE_SIZE * 5);
		IDrawing.setPositionAndWidth(buttonResourcePack, width - SQUARE_SIZE * 5, bottomRowY - SQUARE_SIZE, SQUARE_SIZE * 5);
		IDrawing.setPositionAndWidth(buttonOptions, width - SQUARE_SIZE * 5, bottomRowY, SQUARE_SIZE * 5);
		...
		addRenderableWidget(buttonLineMap);
		addRenderableWidget(buttonRailActions);
		addRenderableWidget(buttonResourcePack);
	...
	public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float delta) {
		...
		final float targetAlpha = showTeleportBar ? 1.0F : 0.0F;
		if (teleportBarAlpha != targetAlpha) {
			final float change = minecraft.getFrameTimeNs() / 1_000_000_000.0F / TELEPORT_BAR_FADE_TIME;
			teleportBarAlpha = targetAlpha > teleportBarAlpha ? Math.min(1.0F, teleportBarAlpha + change) : Math.max(0.0F, teleportBarAlpha - change);
		}

		if (teleportBarAlpha > 0.0F) {
			final int color = 0xFFFFFF | ((int) (teleportBarAlpha * 255) << 24);
			guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TELEPORT_BAR_TEXTURE, width - getTeleportBarWidth(), 0, 0.0F, 0.0F, getTeleportBarWidth(), height, getTeleportBarWidth(), height, 110, 250, color);

			if (!teleportBarTargetText.isEmpty()) {
				final int barLeft = width - getTeleportBarWidth();
				final int barWidth = getTeleportBarWidth();
				final int textLeft = barLeft + (int) (barWidth * 0.1F);
				final int textRight = barLeft + (int) (barWidth * 0.9F);
				final int textTop = (int) (height * 0.115F);
				final int textBottom = (int) (height * 0.14F);
				final FormattedCharSequence orderedText = Text.literal(teleportBarTargetText).getVisualOrderText();
				final int textX = textLeft + (textRight - textLeft - font.width(orderedText)) / 2;
				final int textY = textTop + (textBottom - textTop - IGui.LINE_HEIGHT) / 2;
				guiGraphics.text(font, orderedText, textX, textY, 0x696A76 | ((int) (teleportBarAlpha * 255) << 24), false);
			}
		}

		if (showTeleportBar && (isMouseOverTeleportButton(mouseX, mouseY) || isMouseOverTeleportCloseButton(mouseX, mouseY))) {
	...
	public boolean mouseClicked(MouseButtonEvent event, boolean isDoubleClick) {
		teleportBarMouseDragged = false;
		teleportBarWasOpenOnPress = showTeleportBar;
		suppressTeleportBarOpen = false;
		return super.mouseClicked(event, isDoubleClick);
	}

	@Override
	public boolean mouseDragged(MouseButtonEvent event, double deltaX, double deltaY) {
		teleportBarMouseDragged = true;
		return super.mouseDragged(event, deltaX, deltaY);
	}

	@Override
	public boolean mouseReleased(MouseButtonEvent event) {
		if (showTeleportBar && teleportBarWasOpenOnPress && !teleportBarMouseDragged) {
			if (isMouseOverTeleportButton(event.x(), event.y())) {
				onTeleportBarButtonClicked();
				return true;
			} else if (isMouseOverTeleportCloseButton(event.x(), event.y())) {
				closeTeleportBar();
				return true;
			} else if (event.x() < width - getTeleportBarWidth()) {
				closeTeleportBar();
			}
		}
	...
	private void closeTeleportBar() {
		showTeleportBar = false;
		suppressTeleportBarOpen = true;
		toggleButtons();
	}

	private int getTeleportBarWidth() {
		return height * 490 / 1070;
	}

	private boolean isMouseOverTeleportCloseButton(double mouseX, double mouseY) {
		final int barWidth = getTeleportBarWidth();
		return mouseX >= width - barWidth * 0.1F && mouseY <= height * 0.1F;
	}

	private boolean isMouseOverTeleportButton(double mouseX, double mouseY) {
		final int barLeft = width - getTeleportBarWidth();
		final int barWidth = getTeleportBarWidth();
		return mouseX >= barLeft + barWidth * 0.0F && mouseX <= barLeft + barWidth * 1.0F && mouseY >= height * 0.90F && mouseY <= height * 1.0F;
	}

	private void onTeleportBarButtonClicked() {
		showTeleportBar = false;
		toggleButtons();
		minecraft.setScreen(null);
		if (teleportBarTargetPos != null && minecraft.level != null && minecraft.getConnection() != null) {
			final int targetX = (int) Math.floor(teleportBarTargetPos.getA());
			final int targetZ = (int) Math.floor(teleportBarTargetPos.getB());
			final int targetY = minecraft.level.getHeight(Heightmap.Types.MOTION_BLOCKING, targetX, targetZ) + 1;
			minecraft.getConnection().sendCommand("tp @s " + targetX + " " + targetY + " " + targetZ);
		}
	}
	...
	private void onGeneralMapClicked(Tuple<Double, Double> mouseWorldPos) {
		MTR.LOGGER.info("General map clicked! World Pos: " + mouseWorldPos.getA() + ", " + mouseWorldPos.getB());
		if (!suppressTeleportBarOpen) {
			showTeleportBar = true;
			teleportBarTargetPos = mouseWorldPos;
			teleportBarTargetText = String.format("X: %d  Z: %d", (int) Math.round(mouseWorldPos.getA()), (int) Math.round(mouseWorldPos.getB()));
			toggleButtons();
		}
	}
	...
	private void toggleButtons() {
		...
		final boolean interactive = !showTeleportBar;
		...
		buttonTabStations.active = interactive && selectedTab != SelectedTab.STATIONS;
		buttonTabRoutes.active = interactive && selectedTab != SelectedTab.ROUTES;
		buttonTabDepots.active = interactive && selectedTab != SelectedTab.DEPOTS;
		buttonAddStation.active = interactive;
		buttonAddRoute.active = interactive;
		buttonAddDepot.active = interactive;
		buttonDoneEditingStation.active = interactive && AreaBase.nonNullCorners(editingArea);
		buttonDoneEditingRoute.active = interactive;
		buttonDoneEditingRouteDestination.active = interactive;
		buttonZoomIn.active = interactive;
		buttonZoomOut.active = interactive;
		buttonLineMap.active = interactive;
		buttonRailActions.active = interactive;
		buttonResourcePack.active = interactive;
		buttonOptions.active = interactive;
		colorSelector.active = interactive;
