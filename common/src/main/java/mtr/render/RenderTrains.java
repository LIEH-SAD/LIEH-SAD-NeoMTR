...
public class RenderTrains implements IGui {
	...
	private static boolean prevRenderColors;
	...
	/** Maximum XZ distance in blocks for the real-time rail preview; longer rails are rejected to avoid excessive geometry. */
	private static final double MAX_PREVIEW_RAIL_LENGTH = 200;
	...
	public static void simulate() {
		...
		ClientData.PREVIEW_RAILS.forEach(rail -> renderRailStandard(level, rail, 0.0625F + SMALL_OFFSET, true, 0.5F, 0x80FFFFFF));

		ClientData.LIFTS.forEach(lift -> lift.tickClient(level, newLastFrameDuration));

		// --- Start Real-time Rail Preview Logic ---
		if (player != null) {
			ItemStack mainHandStack = player.getMainHandItem();
			if (mainHandStack.getItem() instanceof ItemRailModifier item) {
				// Start is the previously selected node; end is the aimed rail node or the player
				BlockPos posStart = mainHandStack.get(DataComponentTypes.START_POS.get());
				if (posStart != null) {
					BlockPos posEnd = getRailPreviewEndPos(player, posStart);
					if (!posStart.equals(posEnd)) {
						// Once the second node has been clicked the rail already exists on the
						// client (via the create rail packet), so don't draw the preview on top of it.
						// This also clears the preview as soon as the connection is made, even if the
						// client's START_POS component has not been synced back yet.
						if (RailwayData.containsRail(ClientData.RAILS, posStart, posEnd)
								|| RailwayData.containsRail(ClientData.RAILS, posEnd, posStart)) {
							return;
						}

						// Use the rail type of the held connector so the preview matches the real rail style
						final RailType previewRailType = item.getRailType();
						if (previewRailType == null) {
							return; // Not a rail connector (e.g. rail remover), no preview
						}

						// Reject previews that are too long to prevent excessive geometry generation
						final int diffX = posEnd.getX() - posStart.getX();
						final int diffZ = posEnd.getZ() - posStart.getZ();
						if (Math.sqrt((double) diffX * diffX + (double) diffZ * diffZ) > MAX_PREVIEW_RAIL_LENGTH) {
							return;
						}

						final BlockState stateStart = level.getBlockState(posStart);
						final float angleDifference = (float) Math.toDegrees(Math.atan2(diffZ, diffX));

						// Rail heading at the start node follows the node's actual orientation (same as MTR's placement logic)
						float rawStart = getRawAngleDegrees(level, posStart, stateStart);
						if (Float.isNaN(rawStart)) {
							rawStart = angleDifference;
						}
						// Rail heading at the end: the aimed node's actual orientation, or the player's facing when aiming at nothing
						final BlockState stateEnd = level.getBlockState(posEnd);
						float rawEnd;
						if (stateEnd.getBlock() instanceof BlockNode) {
							final float nodeAngle = getRawAngleDegrees(level, posEnd, stateEnd);
							rawEnd = Float.isNaN(nodeAngle) ? angleDifference : nodeAngle;
						} else {
							rawEnd = player.getYRot() + 90;
						}

						// Apply MTR's orientation flip rules so the curve leaves the start node and enters the end node correctly
						...
						final TransportMode transportMode = mainHandStack.getOrDefault(DataComponentTypes.TRANSPORT_TYPE.get(), TransportMode.TRAIN);

						// Generate both directions with MTR's original rail geometry (curves included)
						Rail previewRail1 = new Rail(posStart, railAngleStart, posEnd, railAngleEnd, previewRailType, transportMode);
						Rail previewRail2 = new Rail(posEnd, railAngleEnd, posStart, railAngleStart, previewRailType, transportMode);

						// Render the preview bypassing the NTE rail dispatcher (vanilla MTR rendering)
						renderPreviewRailStandard(level, previewRail1, 0.0625F + SMALL_OFFSET);
						renderPreviewRailStandard(level, previewRail2, 0.0625F + SMALL_OFFSET);
					}
				}
			}
		}
		// --- End Real-time Rail Preview Logic ---
	}
	...
	public static void render(float tickDelta, PoseStack poseStack, MultiBufferSource vertexConsumers) {
		...
		if (prevRenderColors != renderColors) {
			prevRenderColors = renderColors;
			}
		...
		ClientData.RAILS.forEach((startPos, railMap) -> railMap.forEach((endPos, rail) -> {
			...
			switch (rail.transportMode) {
				case TRAIN:
					renderRailStandard(world, rail, 0.0625F + SMALL_OFFSET, renderColors, 1, rail.railType.color);
					if (renderColors) {
				...
				case BOAT:
					if (renderColors) {
						renderRailStandard(world, rail, 0.0625F + SMALL_OFFSET, true, 0.5F, rail.railType.color);
						renderSignalsStandard(world, poseStack, vertexConsumers, rail, startPos, endPos);
					}
					break;
				case CABLE_CAR:
					if (rail.railType.hasSavedRail || rail.railType == RailType.CABLE_CAR_STATION) {
						renderRailStandard(world, rail, 0.25F + SMALL_OFFSET, renderColors, 0.25F, "mtr:textures/block/metal.png", 0.25F, 0, 0.75F, 1, rail.railType.color);
					}
					if (renderColors && !rail.railType.hasSavedRail) {
						renderRailStandard(world, rail, 0.5F + SMALL_OFFSET, true, 1, "mtr:textures/block/one_way_rail_arrow.png", 0, 0.75F, 1, 0.25F, rail.railType.color);
					}
				...
				case AIRPLANE:
					if (renderColors) {
						renderRailStandard(world, rail, 0.0625F + SMALL_OFFSET, true, 1, rail.railType.color);
						renderSignalsStandard(world, poseStack, vertexConsumers, rail, startPos, endPos);
					} else {
						renderRailStandard(world, rail, 0.0625F + SMALL_OFFSET, false, 0.25F, "textures/block/iron_block.png", 0.25F, 0, 0.75F, 1, rail.railType.color);
					}
					break;
			}
		}));
	...
	private static void renderRailStandard(Level world, Rail rail, float yOffset, boolean renderColors, float railWidth, int color) {
		renderRailStandard(world, rail, yOffset, renderColors, railWidth, renderColors && rail.railType == RailType.QUARTZ ? "mtr:textures/block/rail_preview.png" : "textures/block/rail.png", -1, -1, -1, -1, color);
	}

	/**
	 * Vanilla MTR rail preview rendering that bypasses the NTE rail dispatcher.
	 * The NTE mixin only intercepts {@link #renderRailStandard(Level, Rail, float, boolean, float, int)}, so this
	 * method with a different name keeps preview rails visible when NTE's OBJ rendering is enabled.
	 */
	private static void renderPreviewRailStandard(Level world, Rail rail, float yOffset) {
		...
		rail.render((x1, z1, x2, z2, x3, z3, x4, z4, y1, y2) -> {
			...
			final String texture = rail.railType == RailType.QUARTZ ? "mtr:textures/block/rail_preview.png" : "textures/block/rail.png";
			scheduleRender(Identifier.parse(texture), false, QueuedRenderLayer.EXTERIOR, (matrices, vertexConsumer) -> {
				IDrawing.drawTexture(matrices.last(), vertexConsumer, (float) x1, (float) y1 + yOffset, (float) z1, (float) x2, (float) y1 + yOffset + SMALL_OFFSET, (float) z2, (float) x3, (float) y2 + yOffset, (float) z3, (float) x4, (float) y2 + yOffset + SMALL_OFFSET, (float) z4, 0, 0.1875F + textureOffset, 1, 0.3125F + textureOffset, Direction.UP, color, light2);
				IDrawing.drawTexture(matrices.last(), vertexConsumer, (float) x2, (float) y1 + yOffset + SMALL_OFFSET, (float) z2, (float) x1, (float) y1 + yOffset, (float) z1, (float) x4, (float) y2 + yOffset + SMALL_OFFSET, (float) z4, (float) x3, (float) y2 + yOffset, (float) z3, 0, 0.1875F + textureOffset, 1, 0.3125F + textureOffset, Direction.UP, color, light2);
			});
		}, -0.5F, 0.5F);
	}

	/**
	 * Determine the preview rail destination: the aimed rail node when the player's crosshair is
	 * pointing at a rail node other than the anchor, otherwise the player's current position
	 * (the player acts as a movable virtual node). Returns null when no anchor is selected.
	 */
	public static BlockPos getRailPreviewEndPos(LocalPlayer player, BlockPos posStart) {
		if (posStart == null) {
			return null;
		}
		final HitResult hitResult = player.pick((float) MAX_PREVIEW_RAIL_LENGTH, 0.0F, false);
		if (hitResult.getType() == HitResult.Type.BLOCK) {
			final BlockPos hitPos = ((BlockHitResult) hitResult).getBlockPos();
			if (!hitPos.equals(posStart) && player.level().getBlockState(hitPos).getBlock() instanceof BlockNode) {
				return hitPos;
			}
		}
	...
	/**
	 * Get the raw orientation angle in degrees of a rail node, matching MTR's placement logic.
	 * Returns NaN when the node is a free node with undetermined orientation.
	 */
	private static float getRawAngleDegrees(Level world, BlockPos pos, BlockState state) {
		if (state.getBlock() instanceof BlockFreeNode) {
			final float deg = BlockFreeNode.getRawAngleDegrees(world, pos);
			if (!Float.isNaN(deg)) {
				return deg;
			}
		}
	...
	private static void renderRailStandard(Level world, Rail rail, float yOffset, boolean renderColors, float railWidth, String texture, float u1, float v1, float u2, float v2, int color) {
		final int maxRailDistance = UtilitiesClient.getRenderDistance() * 16;
