...
public class GraphicsTexture implements Closeable {
    ...
    public GraphicsTexture(int width, int height) {
        ...
        identifier = Identifier.fromNamespaceAndPath("mtrscripting", String.format("dynamic/graphics/%s", UUID.randomUUID()));
        graphics = bufferedImage.createGraphics();
        Minecraft.getInstance().execute(() -> {
            dynamicTexture = new DynamicTexture(() -> "JCM GraphicsTexture (" + identifier + ")", new NativeImage(width, height, false));
