...
public class RailRenderDispatcher {
    ...
    public boolean registerRail(Rail rail) {
        boolean vanilla = needsVanillaMTRRendering(rail);
        if (rail.railType == RailType.NONE || vanilla) {
            if (rail.transportMode == TransportMode.BOAT) {
            }
            return false;
        }
    ...
    public void drawRails(Level level, BatchManager batchManager, Matrix4f viewMatrix) {
        ...
        for (Iterator<RailChunkBase> it = railChunkList.iterator(); it.hasNext(); ) {
            ...
            if (chunk.isDirty || !chunk.bufferBuilt) {
                // Rebuild every dirty chunk each frame. Capping this to one rebuild per frame leaves
                // the chunks farthest from the camera (typically the last segment of a long rail)
                // unbuilt/unrendered until all nearer chunks are clean, so the last rail section
                // never appears (or appears with a long delay) after a rail is placed or refreshed.
    ...
    public boolean needsVanillaMTRRendering(Rail rail) {
        ...
        List<RailModelRepeater> repeaters = railExtra.getRepeaters();
        if (repeaters == null || repeaters.isEmpty()) {
            return isHoldingMtrRailRelated;
        } else if (repeaters.size() == 1) {
            if (repeaters.getFirst().getPrimaryModelTypeKey().isEmpty()) {
