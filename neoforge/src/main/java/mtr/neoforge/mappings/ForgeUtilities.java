...
public class ForgeUtilities {
	...
	private static final Map<Identifier, List<Supplier<List<ItemStack>>>> CREATIVE_TAB_STACK_SUPPLIERS = new HashMap<>();
	...
	public static void registerCreativeModeTabStacks(Identifier resourceLocation, Supplier<List<ItemStack>> stacksSupplier) {
		CREATIVE_TAB_STACK_SUPPLIERS.computeIfAbsent(resourceLocation, key -> new ArrayList<>()).add(stacksSupplier);
	}
...
//			if (event.getLayer() != VanillaGuiLayers.SCOREBOARD_SIDEBAR) return;
	...
	public static class RegisterCreativeTabs {
		...
		public static void onRegisterCreativeModeTabsEvent(BuildCreativeModeTabContentsEvent event) {
			...
			CREATIVE_TAB_STACK_SUPPLIERS.forEach((resourceLocation, stacksSuppliers) -> {
				CreativeModeTabWrapper creativeModeTabWrapper = CREATIVE_TABS.get(resourceLocation);
				if (creativeModeTabWrapper != null && creativeModeTabWrapper.creativeModeTab.getDisplayName().equals(event.getTab().getDisplayName())) {
					stacksSuppliers.forEach(stacksSupplier -> {
						stacksSupplier.get().forEach(stack -> event.accept(stack, CreativeModeTab.TabVisibility.PARENT_AND_SEARCH_TABS));
					});
				}
			});
		}
